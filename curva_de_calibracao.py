...
st.set_page_config(page_title='Curva de calibração', layout='centered')
...
if st.button("Rodar modelo!", key= 6, type="primary"):
    if len(st.session_state.X) < 2 or len(st.session_state.Y) < 2:
        st.error('Adicione pelo menos dois pares de concentração e absorbância para ajustar a curva.')
    elif len(st.session_state.X) != len(st.session_state.Y):
        st.error('A quantidade de concentrações e absorbâncias deve ser igual.')
    else:
        x = np.array(st.session_state.X, dtype=float)
        y = np.array(st.session_state.Y, dtype=float)

        if np.allclose(x, x[0]):
            st.error('As concentrações não podem ser todas iguais. Informe valores diferentes para calcular a reta.')
        else:
            ...
            ax1.scatter(x, y)
            ax1.set_xlabel('Concentração (U.C.)', fontsize=10, labelpad=7)
            ax1.set_ylabel('Absorbância (adimensional)', fontsize=10, labelpad=7)
            ax1.plot(x, y_est, '-', color='red')
            st.pyplot(fig1)
            ...
            if len(st.session_state.absorbancia) == 0:
                st.info('Adicione pelo menos uma absorbância de amostra para calcular a concentração do analito.')
            elif np.isclose(a, 0):
                st.error('A inclinação da reta está muito próxima de zero, impossibilitando estimar a concentração da amostra.')
